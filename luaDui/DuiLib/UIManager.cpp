...
void CPaintManagerUI::SetResourceZip(LPCTSTR pStrPath, bool bCachedResourceZip)
{
    ...
    if( m_bCachedResourceZip ) {
        ...
        m_hResourceZip = (HANDLE)OpenZip((void*)sFile.GetString(), 0, 2);
    }
}
    ...
    case WM_PAINT:
        {
            ...
            if( m_bOffscreenPaint && m_hbmpOffscreen == NULL )
            {
				...
				m_hbmpOffscreen = ::CreateDIBSection(NULL, &Bmi, DIB_RGB_COLORS, (void **)&pBits, NULL, 0);
...
//                 }
				RECT rcPaint2 = ps.rcPaint;
				if (::IsRectEmpty(&rcPaint2))
				{
					::GetClientRect(m_hWndPaint, &rcPaint2);
				}
                m_pRoot->DoPaint(m_hDcOffscreen, rcPaint2);
                for( int i = 0; i < m_aPostPaintControls.GetSize(); i++ ) {
                    CControlUI* pPostPaintControl = static_cast<CControlUI*>(m_aPostPaintControls[i]);
                    pPostPaintControl->DoPostPaint(m_hDcOffscreen, rcPaint2);
                }
                ::RestoreDC(m_hDcOffscreen, iSaveDC);
				if (m_bAlphaBackground)
				{	
					LONG dwExStyle = GetWindowLong(m_hWndPaint, GWL_EXSTYLE);
...
// 					WCHAR cBuf[2000] = {0};
// 					wsprintf(cBuf, _T("%d,%d,%d,%d,%d\n"), ptWinPos.x, ptWinPos.y, nWid, nHei,m_hDcOffscreen);
// 					::OutputDebugString(cBuf);

					//HDC hDC = ::GetDC(m_hWndPaint);
					::UpdateLayeredWindow(m_hWndPaint, ps.hdc, &ptWinPos, &sizeWindow, m_hDcOffscreen, &pt, NULL, &bf, ULW_ALPHA);
...
bool CPaintManagerUI::SendMsg(const LPMSG pMsg)
{
	// Pretranslate Message takes care of system-wide messages, such as
	// tabbing and shortcut key-combos. We'll look for all messages for
	// each window and any child control attached.
            ...
            if( data != NULL ) {
                if( !data->sResType.IsEmpty() ) {
                    if( isdigit(*bitmap) ) {
                        LPTSTR pstr = NULL;
                        int iIndex = _tcstol(bitmap, &pstr, 10);
                        pNewData = CRenderEngine::LoadImage(iIndex, data->sResType.GetString(), data->dwMask);
                    }
                }
                else {
