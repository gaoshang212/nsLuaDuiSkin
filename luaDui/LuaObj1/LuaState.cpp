...
int LuaState::setPath(const char* path)
{
	LuaTable package = getGlobal("package");
	...
	if (package.isValid())
	{
		...
		std::string newPath = std::string(spath) + ";" + std::string(cpath);
		package.setTable("path", newPath.c_str());
	}
