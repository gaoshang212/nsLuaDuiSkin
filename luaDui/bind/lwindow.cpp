...
namespace lbind
{
	...
	CDuiString skinXml = param["res"].toLPCTSTR();
	...
	wnd->Create(parent ? parent->GetHWND() : NULL, CDuiString(param["name"].toLPCTSTR()), wndType, WS_EX_STATICEDGE | WS_EX_APPWINDOW, param["x"].toInt(), param["y"].toInt(), param["w"].toInt(), param["h"].toInt());
	...
	LBIND_END_DEFINE_FUNC
		...
		CControlUI* ctl = pThis->FindControl(CDuiString(arg[2].toLPCTSTR()));
	if (ctl)
