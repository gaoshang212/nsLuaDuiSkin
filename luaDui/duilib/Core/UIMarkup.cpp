	...
	bool CMarkup::_Parse(LPTSTR& pstrText, ULONG iParent)
	{
		...
		for (; ; )
		{
			...
			*pstrText++ = _T('\0');
			...
			// Parse attributes
			if (_tcsnccmp(pstrText, _T("![CDATA["), 8) != 0 && !_ParseAttributes(pstrText))
				return false;
			_SkipWhitespace(pstrText);
			...
			else
			{
				...
				if (pstrText[0] == _T('<') && pstrText[1] != _T('/') && _tcsncmp(pstrText, _T("<![CDATA["), 9) != 0)
				{
					if (!_Parse(pstrText, iPos))
						return false;
				}

				if (_tcsncmp(pstrText, _T("<![CDATA["), 9) == 0)
				{
					pstrText += 9;
					_ParseData(pstrText, pstrDest, _T(']'));
					pstrText += 3;
					_SkipWhitespace(pstrText);
				}
