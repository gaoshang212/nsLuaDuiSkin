...
namespace DuiLib {
	...
	CControlUI* CDialogBuilder::_Parse(CMarkupNode* pRoot, CControlUI* pParent, CPaintManagerUI* pManager)
	{
		...
		for (CMarkupNode node = pRoot->GetChild(); node.IsValid(); node = node.GetSibling()) {
			...
			// Add children
			if (node.HasChildren()) {
				CMarkupNode child = node.GetChild();
				if(_tcsicmp(child.GetName(),_T("Event") )== 0 )
				{
					...
					if (L)
					{
						int nAttributes = child.GetAttributeCount();
						for (int i = 0; i < nAttributes; i++) {
							UTF16To8(nameBuf, (unsigned short*)child.GetAttributeName(i), sizeof(nameBuf));
							UTF16To8(valueBuf, (unsigned short*)child.GetAttributeValue(i), sizeof(valueBuf));
						...
						//CMarkupNode node = pRoot->GetChild(); node.IsValid(); node = node.GetSibling()
						for (CMarkupNode evNode = child.GetChild(); evNode.IsValid(); evNode = evNode.GetSibling())
						{
							UTF16To8(nameBuf, (unsigned short*)evNode.GetName(), sizeof(nameBuf));
							int len = UTF16To8(NULL, (unsigned short*)evNode.GetValue(), 0);
							char* buf = (char*)dlmalloc(len + 1);
							UTF16To8(buf, (unsigned short*)evNode.GetValue(), len + 1);
